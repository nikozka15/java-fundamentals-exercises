...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(a -> a.getSex().equals(Sex.MALE)));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(a -> (a.getFirstName() + a.getLastName()).length()).sum();
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
      return accounts.stream()
              .sorted(Comparator.comparing((Account account) -> account.getFirstName()).thenComparing(account1 -> account1.getLastName())).toList();
    }
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream().collect(Collectors.toMap(Account::getId, identity()));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        System.out.println(nameLengthBound);
        System.out.println(accounts.stream()
                .flatMap(a -> Stream.of(a.getFirstName(), a.getLastName()))
                .map(a ->a.toLowerCase())
                .filter(l -> l.length() >= nameLengthBound).toList());

        System.out.println(accounts.stream()
                .map(a -> (a.getFirstName() + a.getLastName())).map(a ->a.toLowerCase())
                .filter(l -> l.length() >= nameLengthBound).toList());
        System.out.println();
        return accounts.stream()
                .flatMap(a -> Stream.of(a.getFirstName(), a.getLastName()))
//                .map(a -> (a.getFirstName() + a.getLastName()))
                .map(a ->a.toLowerCase())
                .filter(l -> l.length() >= nameLengthBound)
                .flatMapToInt(s ->s.chars())
                .mapToObj( c -> (char ) c)
                .collect(
                        Collectors.groupingBy(identity(), Collectors.counting()));
